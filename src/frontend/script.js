...
async function sendRequest(event) {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/post_forms`, {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      throw new Error(`Ошибка POST: ${response.status} Код`);
    }
...
async function getRequests() {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/get_forms`, {
      method: "GET",
    });

    if (!response.ok) {
      throw new Error(`Ошибка GET: ${response.status} Код`);
    }
